...
//     _ = buf;
//     const decoded = common.decodeTag(try reader.reader.peekByte());
...
//     var r = reader_mod.Reader(.{}).init(&fixed_reader);
//     return try r.read();
// }
...
//     const start_pos = reader.reader.end;
...
//         var fixed_reader = Io.Reader.fixed(encoded_buf);
//         var reader = reader_mod.Reader(.{}).init(&fixed_reader);
...
//     const open_start = fixed_reader.end;
//     _ = try reader.read();
//     try writer.raw.writeAll(fixed_reader.buffer[open_start..fixed_reader.end]);
...
//         const key_start = reader.reader.end;
//         const key_val = try reader.read();
//         const key_raw = buf[key_start..reader.reader.end];
...
//         if (match_start == null) {
//             const val_start = reader.reader.end;
//             try reader.skipValue();
//             try writer.raw.writeAll(buf[val_start..reader.reader.end]);
//             continue;
//         }
...
//         const open_start = reader.reader.end;
//         const open_val = try reader.read();
//         try writer.raw.writeAll(buf[open_start..reader.reader.end]);
