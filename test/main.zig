...
test "writer/reader: typedArray roundtrip and skipping" {
    // f32 typedArray
    {
        ...
        var fixed_reader2 = Io.Reader.fixed(written);
        var reader2 = Reader(.{}).init(&fixed_reader2);
...
test "reader: typedArray invalid subtype" {
    ...
    var fixed_reader = Io.Reader.fixed(buf);
    var reader = Reader(.{}).init(&fixed_reader);
...
test "reader: typedArray truncated payload errors" {
    ...
    var fixed_reader = Io.Reader.fixed(buf);
    var reader = Reader(.{}).init(&fixed_reader);
    try std.testing.expectError(error.EndOfStream, reader.read());
}
...
test "reader: typedArray respects max_bytes_length" {
    ...
    var fixed_reader = Io.Reader.fixed(&buf);
    var reader = Reader(.{ .max_bytes_length = 8 }).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(out_buf);
//     var reader = Reader(.{}).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(out_aw.written());
//     var reader = Reader(.{}).init(&fixed_reader);
...
//     try std.testing.expectError(Io.Reader.Error.EndOfStream, out_writer.applyUpdates(truncated, updates[0..]));
// }
...
test "reader: sequential reading" {
    var fixed_reader = Io.Reader.fixed(shared_encoded[0..shared_encoded_len]);
    var reader = Reader(.{}).init(&fixed_reader);
...
test "inspect/allocating: json output" {
    ...
    var fixed_reader = Io.Reader.fixed(shared_encoded[0..shared_encoded_len]);
    var inspector = Inspect(.{}).init(&fixed_reader, &aw.writer, .{});
...
test "inspect: custom options" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var inspector = Inspect(.{}).init(&fixed_reader, &out_fixed, .{
        .indent_size = 2,
        .float_precision = 4,
    });
    try inspector.inspect();
...
test "reader: containerEnd at depth 0" {
    ...
    var fixed_reader = Io.Reader.fixed(malformed);
    var reader = Reader(.{}).init(&fixed_reader);
...
test "reader: nested containerEnd underflow" {
    ...
    var fixed_reader = Io.Reader.fixed(malformed[0 .. written.len + 1]);
    var reader = Reader(.{}).init(&fixed_reader);
...
test "reader: malformed varIntBytes length" {
    ...
    var fixed_reader = Io.Reader.fixed(&malformed);
    var reader = Reader(.{ .max_bytes_length = 1024 * 1024 }).init(&fixed_reader);
    try std.testing.expectError(error.BytesTooLong, reader.read());

    // Without limit, returns EndOfStream
    var fixed_reader2 = Io.Reader.fixed(&malformed);
    var reader2 = Reader(.{}).init(&fixed_reader2);
    try std.testing.expectError(error.EndOfStream, reader2.read());
}
...
test "reader: max_depth limit enforced" {
    ...
    // Set max_depth to 5
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var reader = Reader(.{ .max_depth = 5 }).init(&fixed_reader);
...
test "reader: max_depth null allows unlimited depth" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_aw.written());
    var reader = Reader(.{ .max_depth = null }).init(&fixed_reader);
...
test "reader: max_bytes_length limit enforced for varIntBytes" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var reader = Reader(.{ .max_bytes_length = 50 }).init(&fixed_reader);
...
test "reader: max_bytes_length null allows large bytes" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var reader = Reader(.{ .max_bytes_length = null }).init(&fixed_reader);
...
test "reader: max_array_length limit enforced" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var reader = Reader(.{ .max_depth = 100, .max_array_length = 50 }).init(&fixed_reader);
...
test "reader: max_object_size limit enforced" {
    ...
    var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
    var reader = Reader(.{ .max_depth = 100, .max_object_size = 50 }).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(enc_fixed.buffered());
//     var reader = Reader(.{ .max_depth = 10, .max_object_size = 2 }).init(&fixed_reader);
...
//     try std.testing.expectEqual(@as(usize, 0), fixed_reader.end);
// }
...
//     var fixed_reader = Io.Reader.fixed(truncated);
//     var reader = Reader(.{}).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(fixed.buffered());
//     var reader = Reader(.{ .max_depth = 2 }).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(fixed.buffered());
//     var reader = Reader(.{ .max_depth = 10, .max_array_length = 10 }).init(&fixed_reader);
...
//     var fixed_reader = Io.Reader.fixed(fixed.buffered());
//     var reader = Reader(.{ .max_depth = 10, .max_object_size = 10 }).init(&fixed_reader);
